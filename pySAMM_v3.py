# pySAMM_v3 allows one to change when BZ turns on
...
tBz = 3.04350e-6 + (1.0/3.0)*(2.0*1.e-9)
# tBz = 3.050e-6
...
class InputDeck(HasTraits):
    ...
    save_filename = Dict({'param_file':'string'}) # should not be used if calling 'fname'!
    ...
    param_file = Str(default_value='/Users/thomoor/data/run_parameters/pysamm/default.npz', label='parameter file', help='file to store and read the parameters in/from')
...
# class random_input_ensemble(HasTraits):
#     
#     def __init__(self,path):
#         return
#     
#     def save_instance(self,i):
#         samm_ = OdeSystem(param_file=self.path+str(i), )
#         samm_.save()
#         return
...
class RunSAMM(HasTraits):
    ...
    # param_file = File('/Users/thomoor/data/run_parameters/pysamm/default.npz', label='parameter file', help='file to store and read the parameters in/from')
    # input_param_names = List(['VI_mode','tmin','tmax','deltat','liner_material','nernst','laserabs','n_endloss','AR','rl0','h','Nls','Bz0','Troom','rLEC','rdump','fdops','Adops','Zdops',
    #                         'Nr','rhog0','ftrit','Zdt','deltazlec','tzbl','tauzbl','Ezbl','lambda_b','xi_b','fsharp','bestfocus','dolew','Taur','Z0','L','L0','C','Rloss'])
    # 
...
def run_stuff():
    ...
    laserabs_ = 1
    nernst_ = 1
    rphf_ = 0.645/2.0 # based on 1.5 mm DPP beam diameter
    # rphf_ = 0.645 # based on 3.0 mm DPP beam diameter
    rLEC_ = rphf_*2.325e-3
    # tzbl_ = np.linspace(3043.5,3095.0,10)*1.0e-9
    # Ezbl_ = np.array([200.0, 300.0, 400.0, 750.0, 1.5e3])
    # Ezbl_ = np.array([500.0, 1.0e3, 2.0e3, 3.0e3, 4.0e3])
    Ezbl_ = np.array([200.0, 300.0, 400.0, 500.0, 750.0, 1.0e3, 1.5e3, 2.0e3, 3.0e3, 4.0e3])
    # Ezbl_ = np.ones(len(tzbl_))*2.0e3
    Nls_ = 20
    runs = {}
    
    # for i in range(len(Ezbl_)):
    #     runs[str(int(Ezbl_[i]))] = run_pySAMM(Bz0=Bz0_, h=h_, rhog0=rhog0_, Ezbl=Ezbl_[i], n_endloss=n_endloss_, nernst=nernst_, laserabs=laserabs_, VI_mode='V', Nls=Nls_, rphf=rphf_, rLEC=rLEC_).outputs
    #     print('Ezbl='+str(int(Ezbl_[i])),runs[str(int(Ezbl_[i]))]['rg'][0]/runs[str(int(Ezbl_[i]))]['rg'][np.where(runs[str(int(Ezbl_[i]))]['Ndddot'] == runs[str(int(Ezbl_[i]))]['Ndddot'].max())[0].min()],\
    #         runs[str(int(Ezbl_[i]))]['rg'][0]/runs[str(int(Ezbl_[i]))]['rg'].min(), runs[str(int(Ezbl_[i]))]['Yddn'].max()*1e-13)
    #     strng = 'rhog'+str(rhog0_)+'_h'+str(h_)+'_Bz'+str(Bz0_)+'_Ezbl'+str(Ezbl_[i])+'_nernst'+str(nernst_)+'_endloss'+str(n_endloss_)+'_laserabs'+str(laserabs_)+'_Nls'+str(Nls_)+'_rphf'+str(rphf_)+'_rLEC'+str(rLEC_)
    #     plt.title(strng)
    # np.savez('rhog'+str(rhog0_)+'_h'+str(h_)+'_Bz'+str(Bz0_)+'_nernst'+str(nernst_)+'_endloss'+str(n_endloss_)+'_laserabs'+str(laserabs_)+'_Nls'+str(Nls_)+'_rphf'+str(rphf_)+'_rLEC'+str(rLEC_), runs)
    # 
    # # rphf_ = 0.731/2.0 # based on 1.7 mm DPP beam diameter
    # # rLEC_ = rphf_*2.325e-3
    # n_endloss_ = 0
    # 
    # for i in range(len(Ezbl_)):
...
def parameter_scan_new(path):
    ...
    # rhog0_scan = np.linspace(0.7, 10.0, res)
    rg0_ = 0.002325
    h_scan = np.array([10.0])*1.0e-3
    rhog0_scan = np.array([0.5, 0.7, 1.0, 1.5, 2.0])
    Ezbl_scan  = np.array([0.5, 1.0, 1.5, 2.0, 3.0, 4.0])*1.0e3
    Bz0_scan  = np.array([20.0])
    rphf_scan  = np.array([0.21])
    n_endloss_ = 0
    laserabs_ = 0
    resi = h_scan.size
    resj = rhog0_scan.size
                    ...
                    for m in range(0,resm):
                        ...
                        try:
                            ...
                            try:
                                ...
                                crmax[i,j,k,l,m] = data['rg'][0]/data['rg'].min()
                                index_peak_burn = np.where(data['Yddn'] == data['Yddn'].max())[0].min()
                                crmax[i,j,k,l,m] = data['rg'][0]/data['rg'][index_peak_burn]
...
def load_parameter_scan_from_paper():
    ...
    crmax = np.zeros([resi,resj])
    cr_peak_burn = np.zeros([resi,resj])
    path = '/Users/thomoor/Desktop/runs/parameter_scan_new/'
    for i in range(0,resi):
        for j in range(0,resj):
            filename = 'run_Bz'+str(int(Bz_scan[j]))+'_Ezbl'+str(int(Ezbl_scan[i]))
            print('Running '+filename),
...
def single_run():
    ...
    for i in range(Bz_scan.size):
        ...
        outs = run_.outputs
        ddn[i] = outs['Yddn'].max()
        index_peak_burn = np.where(outs['Nddt'] == outs['Nddt'].max())[0].min()
        crmax[i] = run_.outputs['rg'][0]/run_.outputs['rg'][index_peak_burn]
        pg_stag[i] = (2.0/3.0)*outs['Ethg'][index_peak_burn]/(pi*outs['rg'][index_peak_burn]**2*h)
        path = '/Users/thomoor/Desktop/runs/parameter_scan_kp/'
        save_data(outs, path, filename)
